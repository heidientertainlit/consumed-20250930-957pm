...
serve(async (req) => {
  ...
  try {
    ...
    if (appUserError && appUserError.code === 'PGRST116') {
      ...
      const { data: newUser, error: createError } = await supabaseAdmin
        ...
        .upsert({
          id: user.id,
          email: user.email,
          user_name: user.user_metadata?.user_name || user.email.split('@')[0].split('+')[0] || 'user',
          first_name: user.user_metadata?.first_name || '',
          last_name: user.user_metadata?.last_name || '',
          display_name: user.user_metadata?.user_name || user.email.split('@')[0].split('+')[0] || 'user'
        }, {
          onConflict: 'id',
    ...
    if (req.method === 'GET') {
      ...
      // Persona posts now appear naturally — admin client returns ALL posts chronologically
