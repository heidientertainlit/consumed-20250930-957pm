    ...
    if (source === 'tmdb') {
      ...
      if (response.ok) {
        ...
        mediaDetails = {
          ...
          tmdb_score: data.vote_average || null,
      ...
      if (response.ok) {
        ...
        mediaDetails = {
          ...
          google_books_rating: volumeInfo.averageRating || null,
          google_books_ratings_count: volumeInfo.ratingsCount || 0,
      ...
      if (isISBN) {
        // For ISBN-based lookups, use Google Books first (broader coverage of current titles)
        const googleBooksKey = Deno.env.get('GOOGLE_BOOKS_API_KEY') || '';
        ...
        if (gbResp.ok) {
          ...
          if (vol) {
            ...
            mediaDetails = {
              ...
              google_books_rating: vi.averageRating || null,
              google_books_ratings_count: vi.ratingsCount || 0,
        ...
        // Fallback: Open Library if Google Books returned nothing
        if (!mediaDetails) {
          const olResp = await fetch(`https://openlibrary.org/isbn/${externalId}.json`);
          if (olResp.ok) {
            const bookData = await olResp.json();
