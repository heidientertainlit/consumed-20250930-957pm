...
serve(async (req) => {
  ...
  try {
    ...
    // CRITICAL: Prevent deletion of default/system lists
    if (list.is_default === true) {
      return new Response(JSON.stringify({
        error: 'Cannot delete system lists (Currently, Queue, Finished, etc.)'
      }), {
        status: 403,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      });
    ...
    // Delete the list (CASCADE will handle list_items deletion)
