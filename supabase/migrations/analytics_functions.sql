-- ========================================
-- CONSUMED ANALYTICS FUNCTIONS
-- VC-Ready Metrics for Growth & Engagement
-- ========================================
...
BEGIN
  ...
  WITH user_actions AS (
    -- Track actions from multiple tables
...
-- ========================================
-- 4. ACTIVE ENGAGED USERS (OMTM - 2+ actions/week)
-- ========================================
...
BEGIN
  ...
  SELECT 
    ...
    COUNT(DISTINCT CASE WHEN action_count >= 2 THEN user_id END) as engaged_users,
    ROUND((COUNT(DISTINCT CASE WHEN action_count >= 2 THEN user_id END)::NUMERIC / NULLIF(COUNT(DISTINCT user_id), 0)) * 100, 2) as engagement_rate
...
BEGIN
  ...
  UNION ALL
  SELECT 
    'Played Game (Trivia/Poll/Prediction)',
    COUNT(DISTINCT user_id),
    ROUND((COUNT(DISTINCT user_id)::NUMERIC / NULLIF(total_signups, 0)) * 100, 2)
...
BEGIN
  ...
  SELECT 
    'Total Friendships',
    COUNT(*)::NUMERIC / 2 -- Divide by 2 because friendships are bidirectional
  FROM friendships
  WHERE status = 'accepted';
END;
$$ LANGUAGE plpgsql;
...
BEGIN
  ...
  -- Engagement rate (users with 2+ actions this week)
  WITH user_weekly_actions AS (
  ...
  -- Average actions per user (last 7 days)
  WITH user_action_counts AS (
