-- The live schema stores user_last_activity as a derived view, and the
-- legacy prediction-comment vote table stores comment_id as text while the
-- comment table uses bigint. Keep account deletion executable against those
-- deployed types while retaining the exact-target safeguards.
...
begin
  ...
  -- prediction_comment_votes.comment_id is varchar in the deployed schema,
  -- while prediction_comments.id is bigint; compare their text forms.
  ...
  -- user_last_activity is a GROUP BY view. Remove its session source rows;
  -- list_items and social_posts are removed by their Auth cascades below.
