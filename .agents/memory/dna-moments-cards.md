...
- RLS: dna_moments has ONLY a public-read policy — all client writes are denied. Admin mutations (create/update/delete) go through the generate-dna-moments edge fn (`action: create|update|delete`), which gates on users.is_admin and validates fields server-side. Never add client-side writes to dna_moments.
- Feed placement: DnaMomentCard takes a `slot` prop — each slot shows the Nth unanswered question; cards hide entirely once a user has answered everything (expected, not a bug).
