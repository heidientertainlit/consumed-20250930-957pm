...
- When `type` is provided it narrows which sources are searched (`if (!type || type === 'movie' ...)`).
- **"All" path (no `type`) must NOT collapse to one source on a category keyword.** A removed auto-detect block used to set `type` from a single query keyword (`/\b(song|album|music|listen)\b/` etc.). Those words are often part of a real title ("Song Sung Blue", "The Book Thief"), so collapsing dropped every other source and hid the correct result (e.g. "song sung blues" returned only music, never the movie). Fix: search ALL sources on the "All" path; the relevance scoring already boosts the hinted type by +40 via the independent `queryHas*` flags, so the right category floats up without exclusion. **Why:** category words are ambiguous title tokens, not reliable filters. **How to apply:** never re-add keyword-based hard narrowing to the no-type path; tune ranking instead. Tradeoff: "All" now fans out to all sources (more latency) and exact-title matches may sit at #2 behind a tangential same-word result — a ranking-quality issue, not a break.
