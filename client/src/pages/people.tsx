...
function TribeDetail({ tribe, onBack, membership, personalized, allowOverall }: { tribe: Tribe; onBack: () => void; membership: ReturnType<typeof useMutation<TribesResponse, Error, { slug: string; joined: boolean }>>; personalized: boolean; allowOverall: boolean }) {
  ...
  return <section className="mt-7"><button onClick={onBack} className="mb-5 inline-flex items-center gap-1 text-sm font-bold text-[#543d72]"><ArrowLeft size={16} /> All taste groups</button>
    <div className="overflow-hidden rounded-[22px] border border-[#d8cce1] bg-[#ece6ec]"><div className="bg-[linear-gradient(135deg,#f3edf8_0%,#e9def2_52%,#dfd0eb_100%)] p-5 sm:p-6"><div>{personalized && <TribeLabel>{Math.round(tribe.fit_score)}% overlap</TribeLabel>}<h2 className={`${personalized ? "mt-2" : ""} max-w-xl font-serif text-2xl font-medium leading-[1.08] tracking-[-.04em] text-[#30203f] sm:text-3xl`}>{positioning.title}</h2><p className="mt-3 max-w-xl text-sm leading-5 text-[#5f5665]">{positioning.line}</p></div>
