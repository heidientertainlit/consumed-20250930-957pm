    ...
    <main className="max-w-5xl mx-auto px-4 sm:px-6 pt-6 sm:pt-10">
      ...
      {mode === "tribes" && <section className="mt-7 animate-in fade-in duration-300">
        {activeBand ? <BandDetail band={activeBand} onBack={() => setSelectedBand(null)} /> : <><div className="mb-5"><p className="eyebrow">Personalized affinity</p><h2 className="section-title">Find your people</h2><p className="text-sm text-[#766c76] mt-1 max-w-lg">Not genre groups. These bands reflect how closely your Entertainment DNA lines up with real people.</p></div>
          {tribesQuery.isLoading ? <div className="space-y-3">{[1,2,3,4].map((item) => <div key={item} className="h-32 rounded-[24px] bg-[#e9e0d5] animate-pulse" />)}</div> : tribesQuery.isError ? <div className="surface p-7 text-sm">Your affinity map couldn’t load. <button className="font-black text-[#6547b8]" onClick={() => tribesQuery.refetch()}>Try again</button></div> : !affinity?.ready ? <div className="surface p-9 text-center"><LockKeyhole size={28} className="mx-auto text-[#775cc4] mb-3" /><h3 className="font-black text-lg">Your people are still taking shape.</h3><p className="text-sm text-[#766c76] max-w-sm mx-auto mt-1">{affinity?.readiness?.items_needed ? `Add ${affinity.readiness.items_needed} more titles to unlock personalized affinity.` : "Complete your Entertainment DNA to unlock personalized affinity."}</p><Link href="/me" className="inline-flex mt-5 rounded-xl bg-[#30234f] px-4 py-2.5 text-sm font-black text-white">Build your DNA</Link></div> : <div className="space-y-3">{(affinity.bands || []).map((band) => <button key={band.id} onClick={() => setSelectedBand(band.id)} className="w-full text-left rounded-[24px] border border-[#dfd5c9] bg-[#fffaf2] p-5 sm:p-6 flex items-center gap-4 hover:-translate-y-0.5 hover:shadow-md transition-all"><div className="w-12 h-12 rounded-2xl flex items-center justify-center shrink-0" style={{ background: band.tint, color: band.color }}><Dna size={24} /></div><div className="flex-1 min-w-0"><div className="flex items-center gap-2 flex-wrap"><h3 className="font-black text-lg">{band.label}</h3><span className="text-[11px] font-black" style={{ color: band.color }}>{band.min}–{band.max}%</span></div><p className="text-sm text-[#766c76] mt-1">{band.feeling}</p><div className="mt-2 flex items-center gap-2">{band.people.length > 0 && <div className="flex -space-x-2">{band.people.slice(0, 3).map((person) => <span key={person.id} className="rounded-full border-2 border-[#fffaf2]"><Avatar person={person} size="sm" /></span>)}</div>}<p className="text-xs font-bold text-[#9a8b89]">{band.people.length ? `${band.people.length} people ready to meet` : "No matches here yet"}</p></div></div><ChevronRight size={20} className="text-[#b5aab1]" /></button>)}</div>}
