...
function Matches({ query, more, onSelectPerson, onInvite }: { query: ReturnType<typeof useQuery<Affinity>>; more: ReturnType<typeof useMutation<Affinity, Error, void>>; onSelectPerson: (person: Person) => void; onInvite: () => void }) {
  ...
  return <section className="mt-7">
    {!ordered.length ? <div className="rounded-xl border border-dashed border-[#d6ceda] px-5 py-8 text-sm text-[#746b7b]">No comparisons to show yet. Your matches will arrive as more people build their DNA.</div> :
      <>
        {featured.length > 0 && <div className="mb-9"><div className="mb-3"><h3 className="inline-flex items-center gap-2 font-serif text-[21px] font-medium tracking-[-.015em] text-[#30203f]">Your Closest Matches <Dna size={17} className="shrink-0 text-[#65457b]" /></h3><p className="mt-1 max-w-xl text-[13px] leading-5 text-[#7d7382]">See who shares similar Entertainment DNA.</p></div><div className="grid gap-3 lg:grid-cols-2">{featured.map(({ person, band }, index) => <FeaturedMatch key={person.id} person={person} band={band} index={index} onSelect={onSelectPerson} />)}</div></div>}
