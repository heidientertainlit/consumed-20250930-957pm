...
export default function PeoplePage({ initialTribeId }: { initialTribeId?: string } = {}) {
  ...
  return <div className="min-h-[100dvh] bg-[#fbf8f5] pb-24 text-[#271d3a]">
    ...
    <div className="bg-[#fbf8f5] px-4 pt-4 sm:px-6">
      <nav className="mx-auto flex max-w-5xl gap-2 overflow-x-auto pb-1" aria-label="People sections">
...
function TribeDetail({ tribe, onBack, membership, personalized, allowOverall }: { tribe: Tribe; onBack: () => void; membership: ReturnType<typeof useMutation<TribesResponse, Error, { slug: string; joined: boolean }>>; personalized: boolean; allowOverall: boolean }) {
  ...
  return <section className="mt-7"><button onClick={onBack} className="mb-5 inline-flex items-center gap-1 text-sm font-bold text-[#543d72]"><ArrowLeft size={16} /> All taste groups</button>
    <div className="overflow-hidden rounded-[22px] border border-[#e2dcd5] bg-[#fffdf9]"><div className="bg-[#fffdf9] p-5 sm:p-6"><div>{personalized && <TribeLabel>{Math.round(tribe.fit_score)}% overlap</TribeLabel>}<h2 className={`${personalized ? "mt-2" : ""} max-w-xl font-serif text-2xl font-medium leading-[1.08] tracking-[-.04em] text-[#30203f] sm:text-3xl`}>{positioning.title}</h2><p className="mt-3 max-w-xl text-sm leading-5 text-[#5f5665]">{positioning.line}</p></div>
      ...
      {groupPeople.length > 0 && <button type="button" aria-expanded={showPeople} onClick={() => setShowPeople((open) => !open)} className="mt-4 flex w-full items-center border-t border-[#ded8d2] pt-4 text-left focus:outline-none focus-visible:ring-2 focus-visible:ring-[#67447c]">
        <span className="flex shrink-0 -space-x-2">{groupPeople.slice(0, 4).map((person) => <span key={person.id} className="rounded-full border-2 border-[#fffdf9]"><Avatar person={person} small /></span>)}</span>
      ...
      {recentTakes.length > 0 && <div className="border-t border-[#e4ded8] bg-[#fffdf9] p-6 sm:p-8"><TribeLabel>What they’re talking about</TribeLabel><div className="mt-4 space-y-3">{recentTakes.map((take) => {
