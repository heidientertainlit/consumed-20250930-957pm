...
export default function AdminRoomConversationsPage() {
  ...
  async function publish() {
    ...
    try {
      ...
      setPublishedUrl(`/conversation/${data.takeId}`);
      toast({ title: "Conversation published", description: "The conversation is now live." });
    } catch (error: any) {
  ...
  return <div className="min-h-screen bg-gray-950 text-white"><main className="max-w-4xl mx-auto px-4 py-8">
    ...
    <header className="mb-7"><div className="flex items-center gap-3"><div className="rounded-xl p-2.5 bg-purple-900/40"><MessageSquareText className="text-purple-300" /></div><div><h1 className="text-2xl font-bold">True Crime Conversation</h1><p className="text-sm text-gray-400">Build a reviewed, 20-persona conversation from current sources.</p></div></div></header>
    {roomQuery.isLoading ? <div className="py-16 flex justify-center"><Loader2 className="animate-spin text-purple-400" /></div> : roomQuery.isError || !room ? <div className="rounded-2xl border border-red-900/60 bg-red-950/30 p-6"><p className="font-medium">True Crime conversation setup unavailable</p><p className="text-sm text-gray-400 mt-1">{(roomQuery.error as Error)?.message || "The official True Crime conversation setup could not be loaded."}</p><Button variant="outline" className="mt-4" onClick={() => roomQuery.refetch()}>Try again</Button></div> : <>
      <section className="rounded-2xl border border-gray-800 bg-gray-900 p-5 mb-5 flex flex-wrap justify-between gap-4"><div><p className="text-xs uppercase tracking-wider text-purple-300 font-semibold">Conversation collection</p><h2 className="text-lg font-semibold mt-1">{room.name}</h2><p className="text-sm text-gray-400 mt-1">{room.description || "Official True Crime community"}</p></div><span className="self-start rounded-full bg-purple-500/15 px-3 py-1 text-xs text-purple-200 border border-purple-500/30">Private preview first</span></section>
