...
export default function AdminRoomConversationsPage() {
  ...
  return <div className="min-h-screen bg-gray-950 text-white"><main className="max-w-4xl mx-auto px-4 py-8">
    ...
    {roomQuery.isLoading ? <div className="py-16 flex justify-center"><Loader2 className="animate-spin text-purple-400" /></div> : roomQuery.isError || !room ? <div className="rounded-2xl border border-red-900/60 bg-red-950/30 p-6"><p className="font-medium">True Crime room unavailable</p><p className="text-sm text-gray-400 mt-1">{(roomQuery.error as Error)?.message || "The official True Crime room could not be loaded."}</p><Button variant="outline" className="mt-4" onClick={() => roomQuery.refetch()}>Try again</Button></div> : <>
      ...
      <section className="rounded-2xl border border-gray-800 bg-gray-900 p-5 mb-5"><div className="flex justify-between gap-3 mb-4"><div><h2 className="font-semibold">1. Choose a safe conversation starter</h2><p className="text-sm text-gray-400">Trending options are sourced and checked before generation.</p></div><Button size="sm" variant="ghost" onClick={() => topicsQuery.refetch()}><RefreshCw size={14} className="mr-1" /> Refresh</Button></div>
        {topicsQuery.isLoading ? <div className="py-6 flex justify-center"><Loader2 className="animate-spin text-purple-400" /></div> : topicsQuery.isError ? <p className="rounded-xl bg-red-950/30 p-4 text-sm text-red-300">Trending starters could not be loaded. You can still use a custom safe starter.</p> : suggestions.length === 0 ? <p className="rounded-xl bg-gray-950 p-4 text-sm text-gray-400">No trending starters are available right now. You can still use a custom safe starter.</p> : <div className="space-y-2" role="radiogroup" aria-label="Conversation starters">{suggestions.map(item => <div key={item.id} className={`rounded-xl border p-4 transition-colors ${selectedStarter === item.id && !customStarter ? "border-purple-500 bg-purple-950/30" : "border-gray-800 bg-gray-950 hover:border-gray-700"}`}><label className="flex gap-3 cursor-pointer"><input type="radio" name="conversation-starter" checked={selectedStarter === item.id && !customStarter} onChange={() => { setSelectedStarter(item.id); setCustomStarter(""); }} className="mt-1 accent-purple-500" /><span><span className="font-medium text-sm block">{item.title}</span>{item.summary && <span className="text-sm text-gray-400 mt-1 block">{item.summary}</span>}</span></label><div className="flex flex-wrap gap-x-3 gap-y-1 mt-3 pl-6 text-xs text-gray-500">{item.source?.name && <span>Source: {item.source.url ? <a className="text-purple-300 hover:underline" href={item.source.url} target="_blank" rel="noreferrer">{item.source.name} <ExternalLink className="inline" size={10} /></a> : item.source.name}</span>}{(item.freshness || item.source?.published_at || item.source?.fetched_at) && <span>Freshness: {item.freshness || new Date(item.source?.published_at || item.source?.fetched_at || "").toLocaleDateString()}</span>}{item.safety && <span className="text-emerald-400"><ShieldCheck className="inline mr-1" size={12} />{item.safety}</span>}</div></div>)}</div>}
        <div className="mt-4"><label className="text-xs uppercase tracking-wider text-gray-400">Or write a custom safe starter</label><Textarea value={customStarter} onChange={e => { setCustomStarter(e.target.value); setSelectedStarter(""); }} placeholder="Ask a specific, respectful True Crime discussion question…" className="mt-2 bg-gray-950 border-gray-700 min-h-[84px]" maxLength={500} />{customError ? <p className="text-xs text-red-400 mt-1">{customError}</p> : customStarter && <p className="text-xs text-emerald-400 mt-1"><ShieldCheck className="inline mr-1" size={12} />Starter is valid. Click Generate below to create the private preview.</p>}
          {(customStarter || starter) && <Button onClick={generatePreview} disabled={working !== null || !!customError || (!!personaDeficit && !approval)} className="mt-4 w-full bg-purple-600 hover:bg-purple-700">{working === "preview" ? <><Loader2 className="animate-spin mr-2" size={15} />Building preview…</> : <><Sparkles className="mr-2" size={15} />Generate private preview</>}</Button>}
        </div>
      </section>
