...
import { Vote, Star, Users, UserPlus, ChevronLeft, ChevronRight, Search, SlidersHorizontal } from 'lucide-react';
          ...
          <div className="mb-8">
            ...
            <div className="flex items-center justify-between mb-3">
              <div className="text-sm text-gray-600">
                <span className="font-medium">{currentSlide + 1}</span> of <span>{pollGames.length}</span> polls
              </div>
              <div className="flex items-center gap-2">
                <button
                  onClick={scrollPrev}
                  disabled={!canScrollPrev}
                  className="p-2 rounded-full bg-white border border-gray-200 disabled:opacity-30 disabled:cursor-not-allowed hover:bg-gray-50 transition-all"
            ...
            <div className="overflow-hidden" ref={emblaRef}>
              <div className="flex gap-4">
                {pollGames.map((game: any, index: number) => (
                  <div 
                    key={game.id} 
                              ...
                              {(game.options || []).map((option: any, optIndex: number) => {
                                ...
                                return (
                                  <button
                                    ...
                                    data-testid={`option-${game.id}-${optIndex}`}
                                  >
                                    {optionText}
                                  </button>
                                );
            ...
            <div className="flex justify-center gap-1.5 mt-4">
              {pollGames.map((_: any, index: number) => (
                <button
                  key={index}
