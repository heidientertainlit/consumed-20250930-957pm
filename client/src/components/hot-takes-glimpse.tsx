...
export function HotTakesGlimpse({ variant }: HotTakesGlimpseProps) {
  ...
  // Consumed: gradient background like "More Games"
  // User: white card with gradient header like screenshot 1
  if (variant === 'consumed') {
    return (
      <Card className="bg-gradient-to-br from-purple-600 via-purple-500 to-indigo-500 border-0 rounded-2xl p-4 shadow-lg mb-4">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-sm font-semibold text-white">🏆 Consumed Hot Takes</p>
              <p className="text-[10px] text-white/70">Official spicy opinions</p>
            </div>
          </div>
        ...
        <div className="space-y-2">
          {hotTakes.slice(0, 2).map((take) => (
            <div key={take.id} className="bg-white/15 rounded-xl p-3 backdrop-blur-sm">
              <div className="flex items-start gap-2 mb-2">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-xs text-white/80">{take.user?.display_name || take.user?.username}</p>
  ...
  // User variant: white card with gradient purple header
  return (
    <Card className="bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-sm mb-4">
      {/* Gradient header */}
      <div className="bg-gradient-to-r from-purple-600 to-purple-500 px-4 py-2">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Flame className="w-4 h-4 text-white fill-white" />
            <span className="text-sm font-semibold text-white uppercase tracking-wide">Hot Take</span>
          </div>
          <Link href="/play/hot-takes">
            <div className="flex items-center gap-1 text-white/80 hover:text-white cursor-pointer">
              <span className="text-xs font-medium">See All</span>
      ...
      <div className="p-4 space-y-3">
        {hotTakes.slice(0, 2).map((take) => (
          <div key={take.id} className="border-b border-gray-100 last:border-0 pb-3 last:pb-0">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-gray-900">{take.user?.display_name || take.user?.username}</p>
                <p className="text-sm text-gray-700 line-clamp-2 mt-1">{take.content}</p>
