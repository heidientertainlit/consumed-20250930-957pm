...
export default function FriendsManager({
  ...
  const blockedUsersReady = blockedUsers.isSuccess;
  ...
  const sortedFriends = [...(friendsData?.friends || [])]
    .filter((friendship: any) => friendship.friend?.id !== visibleFeaturedFriend?.id)
    .filter((friendship: any) => !blockedIds.has(friendship.friend?.id))
    .sort((a: any, b: any) => {
  ...
  const blockedPeopleLoading = blockedUsers.isLoading || (blockedUsersReady && isLoadingBlockedProfiles);
  ...
  return (
    ...
    <div className="mb-6">
      ...
      <div>
        ...
        {searchQuery.length > 0 && (
          blockedPeopleLoading ? (
            <div className="mt-3 py-4 text-center text-sm text-[#817686]">Checking blocked people...</div>
          ) : blockedUsers.isError ? (
            <div className="mt-3 py-4 text-center text-sm text-red-700" data-testid="text-blocked-search-error">
              {blockedUsers.error?.message || "Blocked people could not be verified for this account."}
            </div>
            ...
            {searchQuery.length >= 3 ? (
              ...
              ) : searchResults?.users && searchResults.users.filter((searchUser: any) => !blockedIds.has(searchUser.id)).length > 0 ? (
                searchResults.users.filter((searchUser: any) => !blockedIds.has(searchUser.id)).map((searchUser: any) => {
    ...
    <div className="overflow-hidden rounded-[20px] border border-[#e4dedb] bg-[#fffdfb] shadow-[0_7px_18px_rgba(65,49,55,.055)]">
     ...
     {visibleFeaturedFriend && <ClosestFriendCard friend={visibleFeaturedFriend} onBlock={() => setBlockTarget({ id: visibleFeaturedFriend.id, name: visibleFeaturedFriend.user_name })} />}
     {(!visibleFeaturedFriend || sortedFriends.length > 0 || blockedPeopleLoading || blockedUsersError || trustedBlockedUserIds.length > 0) && <section className="mt-7">
      <div className="mb-3 flex items-end justify-between gap-3">
        <div>
            <h3 className="text-[10px] font-medium uppercase tracking-[.18em] text-[#817786]">{visibleFeaturedFriend ? "More friends" : "Your friends"}</h3>
            <p className="mt-0.5 text-xs text-[#7d7382]">{visibleFeaturedFriend ? "Everyone else in your circle." : "Closest DNA matches first."}</p>
        </div>
         {!isLoadingFriends && blockedUsersReady && sortedFriends.length > 0 && (
           <span className="text-xs font-semibold text-[#79618f]">{sortedFriends.length}</span>
        )}
      </div>

       {isLoadingFriends || blockedPeopleLoading ? (
        <div className="space-y-2">
          {[1, 2, 3].map((item) => <div key={item} className="h-[62px] animate-pulse rounded-xl bg-[#e6e0e7]" />)}
        </div>
       ) : blockedUsers.isError ? (
         <div className="rounded-xl border border-red-200 bg-red-50 px-5 py-5 text-sm text-red-700" data-testid="text-blocked-users-error">
           {blockedUsers.error?.message || "Blocked people could not be verified for this account."}
         </div>
       ...
       {!isLoadingFriends && blockedUsersReady && sortedFriends.length > 5 && (
        <button
          type="button"
