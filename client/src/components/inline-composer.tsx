  ...
  return (
    <div className="rounded-2xl shadow-lg bg-gradient-to-r from-purple-600 via-pink-600 to-purple-600 p-6">
      ...
      {composerMode === "prediction" && (
        <div className="mb-4 bg-white/20 backdrop-blur-sm rounded-xl p-4 space-y-3">
          <p className="text-xs text-white font-semibold">Prediction Type</p>
          ...
          {predictionType === "multi-choice" && (
            <div className="space-y-2">
              <p className="text-xs text-white font-semibold">Options</p>
              {predictionOptions.map((option, index) => (
                <input
                  ...
                  className="w-full px-3 py-2 text-sm bg-white border-0 rounded-lg focus:outline-none focus:ring-2 focus:ring-white"
                />
              ))}
            </div>
          )}
      ...
      {composerMode === "poll" && (
        <div className="mb-4 bg-white/20 backdrop-blur-sm rounded-xl p-4 space-y-2">
          <p className="text-xs text-white font-semibold">Poll Options</p>
          {pollOptions.map((option, index) => (
            <div key={index} className="flex gap-2">
              <input
                ...
                placeholder={`Option ${index + 1}`}
                className="flex-1 px-3 py-2 text-sm bg-white border-0 rounded-lg focus:outline-none focus:ring-2 focus:ring-white"
              />
              {pollOptions.length > 2 && (
                <Button
                  ...
                  variant="ghost"
                  size="sm"
                  className="h-9 w-9 p-0 text-white hover:text-white/70"
                >
                  <X className="w-4 h-4" />
                </Button>
              )}
          ...
          {pollOptions.length < 4 && (
            <Button
              ...
              variant="outline"
              size="sm"
              className="w-full text-xs bg-white/30 text-white border-white/50 hover:bg-white/50"
            >
                ...
                <div className="max-h-48 overflow-y-auto space-y-1">
                  {searchResults.slice(0, 8).map((result, idx) => (
                    <button
                      ...
                      className="w-full flex items-center gap-3 p-2 bg-white/50 hover:bg-white rounded-lg text-left transition-colors"
                    >
                      {result.poster_url && (
                        <img 
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          />
          <label htmlFor="spoilers" className="text-xs text-white font-medium">
            Contains spoilers
          </label>
        </div>
        <Button
          ...
          className="bg-white hover:bg-white/90 text-purple-600 font-semibold"
