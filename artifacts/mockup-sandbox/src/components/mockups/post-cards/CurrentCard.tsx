function Poster({ w, h }: { w: number; h: number }) {
  return (
    <div className="shrink-0 rounded-xl overflow-hidden relative" style={{ width: w, height: h, background: "linear-gradient(135deg, #2d1b69 0%, #4c1d95 50%, #1e3a5f 100%)" }}>
      <div className="absolute inset-0 flex flex-col justify-between p-1.5">
        ...
        <div className="flex items-center gap-1">
          <div className="w-4 h-4 rounded bg-black/40 flex items-center justify-center">
            <svg width="8" height="8" viewBox="0 0 24 24" fill="white" opacity="0.9"><rect x="2" y="3" width="20" height="14" rx="2"/><path d="M8 21h8M12 17v4" stroke="white" strokeWidth="2" strokeLinecap="round"/></svg>
          </div>
        </div>
      </div>
      <div className="absolute inset-0 flex items-center justify-center">
        <span className="text-white/30 text-[9px] font-bold tracking-wide rotate-0">PAST LIVES</span>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex items-start justify-center pt-6 px-4">
      <div className="w-full max-w-sm">
        <p className="text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-2 text-center">Current — Full Poster</p>
