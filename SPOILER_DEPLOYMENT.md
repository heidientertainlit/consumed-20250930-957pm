# Spoiler Feature Deployment Guide
...
```sql
-- Add spoiler protection column to social_posts table
ALTER TABLE social_posts ADD COLUMN IF NOT EXISTS contains_spoilers BOOLEAN DEFAULT FALSE;
```
...
```bash
# Deploy share-update function
supabase functions deploy share-update

# Deploy social-feed function
supabase functions deploy social-feed
```

### Option B: Deploy via Supabase Dashboard

Go to **Supabase Dashboard → Edge Functions** and update the following functions:

---

## `share-update` Edge Function

**File:** `supabase/functions/share-update/index.ts`
...
**File:** `supabase/functions/social-feed/index.ts`

**Key Changes:**
- Line 109: Added `contains_spoilers` to SELECT query
- Line 165: Added `containsSpoilers: post.contains_spoilers || false` to response
...
3. ✅ Test creating a post with spoiler checkbox checked
4. ✅ Verify spoiler warning appears in feed
5. ✅ Test "Show Anyway" button reveals content
6. ✅ Test creating post without spoiler checkbox

---

## Why You Don't See It Yet

The spoiler checkbox won't work until you:
1. Run the database migration (adds the column)
2. Deploy the updated edge functions (saves and retrieves the spoiler flag)

The UI changes are already live in your app, but the backend needs to be updated in Supabase.
